...
\author{Curt Lynch && Hunter Johnson}

\IEEEtitleabstractindextext{%
\begin{abstract}
This will serve as the documentation of our CSCI 352 project Temtris.
\end{abstract}
}
...
Temtris is a wpf application that we aim to be Tetris clone. That is, it is a game where you try to fit rows of blocks together at an ever increasing pace until the screen inevitably fills and you lose.

We are making Temtris as a duo as part part of our CSCI 352 class. That being the case, we still want Temtris to be a fun and interesting addition to the world of Tetris.
...
This is the first moderately sized project as well as the first time we've really used a ui framework. I honestly expect more fighting with wpf than actual issues with game logic.

\section{Scope}
Our initial goal is to implement the following typical parts of a Tetris-like:
Have controllable game pieces that fall into place.
Have rows that disappear and collapse when they fill up.
Keep score or track time in some way.
Display the next two pieces to spawn and allow the player to swap them.

For stretch goals we would like to add the following:
Multiplayer? Have at least two clients with a competitive game mode over LAN or just sharing a keyboard.
